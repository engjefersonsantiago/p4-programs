...
P4C_EXEC=p4c-bm2-ss
SWITCH_EXEC=simple_switch
CLI_EXEC=simple_switch_CLI
...
set -m
$P4C_EXEC p4src/div.p4 -o div.json --emit-externs
if [ $? -ne 0 ]; then
...
sudo $SWITCH_EXEC >/dev/null 2>&1
sudo $SWITCH_EXEC div.json \
    -i 0@veth0 -i 1@veth2 -i 2@veth4 -i 3@veth6 -i 4@veth8 \
    --nanolog ipc:///tmp/bm-0-log.ipc --log-console \
    --pcap -- \
    --load-modules=./user_externs/div.so &
$CLI_EXEC --json div.json < commands.txt
echo "READY!!!"
fg
